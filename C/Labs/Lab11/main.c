...
void alterarRecebimento(FILE *recebimentos); //Questao 6
void mostrarRecPorVenci(FILE *recebimentos); //Questao 7
void mostrarRecPorCli(recebimentos); //Questao 8
    ...
    while(opcao!=0)
    {
        ...
        switch (opcao)
        {
            ...
            case 6:
                ...
                break;
            case 7:
                mostrarRecPorVenci(recebimentos);
                break;
            case 8:
                mostrarRecPorCli(recebimentos);
                break;
            case 0:
                printf("Saindo...");
                break;
...
void incluirRecebimentos(FILE *clientes, FILE *recebimentos)
{
    ...
    fprintf(recebimentos,"%d|%d:%.2f-e%d/%d/%d-v%d/%d/%d#%d*\n", codigo, numDoc, valorDoc, emiDia, emiMes, emiAno, vencDia, vencMes, vencAno, codigo);
    fclose(recebimentos);
...
void alterarRecebimento(FILE *recebimentos)
{
    int codigo, verificar=0;
    char codigoChar[5], ler[1000];
    int pos=0, i=0, y=0;
    ...
    do
    {
        printf("Qual o codigo do cliente? ");
        scanf("%d", &codigo);
        while (fgets(codigoChar,5,recebimentos)!=NULL && verificar==0)
        {
            if (codigo==atoi(codigoChar))
            {
                verificar=1;
                pos = ftell(recebimentos);
                fseek(recebimentos,pos+1,SEEK_SET);
                do
                {
                    y++;
                }while(fgetc(recebimentos)!='*');
                fseek(recebimentos,pos+1,SEEK_SET);
                for(i=0;i<y;i++)
                {
                    fputc('\0', recebimentos);
                }
                float valorDoc;
                int emiDia, emiMes, emiAno;
                int vencDia, vencMes, vencAno;
                ...
                printf("Ano: ");
                scanf("%d", &vencAno);
                fprintf(recebimentos,"%d:%.2f-e%d/%d/%d-v%d/%d/%d#%d*", docCode, valorDoc, emiDia, emiMes, emiAno, vencDia, vencMes, vencAno, codigo);
                fclose(recebimentos);
                recebimentos = fopen("recebimentos.txt", "r+");
                fseek(recebimentos,0,SEEK_END);
                fputc('\n', recebimentos);
                fclose(recebimentos);
            }
            fgets(ler,1000,recebimentos);
        }
        if (verificar==0)
            printf("Informe um codigo cadastrado\n");
        fseek(recebimentos,0,SEEK_SET);
...
void mostrarRecPorVenci(FILE *recebimentos)
{
    recebimentos = fopen("recebimentos.txt", "r+");
    int dia, mes, ano;
    char linha[999];
    ...
    printf("Ano: ");
    scanf("%d", &ano);
    while (!feof(recebimentos))
    {
        fgets(linha,999,recebimentos);
        printf("\n%s",linha);
    }
    fclose(recebimentos);
    printf("\nFIM");
...
void mostrarRecPorCli(FILE *recebimentos)
{
    int codigo, verificar=0;
    char codigoChar[5], ler[1000];
    int pos=0, i=0, y=0;
